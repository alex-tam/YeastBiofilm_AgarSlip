...
function draw_solution(i, ξ, ξo, τ, S, h, ϕ, gs, gso, gb, u, a)
    ...
    plot!(S.*ξ, u, linecolor = :orange, linewidth = 3, label=L"$u$")
...
function thinfilm_slip(par, ex, output::Bool)
    ...
    if output == true
        plot(τ, S, xlabel = L"$\tau$", ylabel = L"$S(\tau)$", linecolor = :black, linewidth = 3, grid = false, margin=5mm, legend = false, xlims=(0, 1.05*maximum(τ)), ylims=(0, par.L))
        scatter!(ex.t, ex.w, marker =:xcross, markersize = 10, markerstrokewidth = 3, linecolor =:black, label = "0.6% Agar")
