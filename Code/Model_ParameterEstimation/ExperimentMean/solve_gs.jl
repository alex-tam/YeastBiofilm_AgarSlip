...
function solve_gs(par, dτ, dξ, ξ, ξo, gs, gso, gb, u, S)
    ...
    for iterations = 1:10
        ...
        gs_pert = solve_gs_inner(par, dτ, dξ, gr + par.δ, ξ, gs, gb, u, S)
        gso_pert = solve_gs_outer(par, dτ, dξo, gr + par.δ, ξo, gso, u, S)
    ...
    if any(gs_new .< 0) || any(gso_new .< 0)
        @printf("Error: Non-physical nutrient concentration in substratum.\n")
        flag = true
        return gs, gso, flag
    end
...
function solve_gs_outer(par, dτ, dξo, gr, ξo, gs, u, S)
    ### Build matrix ##
    A = Matrix{Float64}(undef, par.Nξo, par.Nξo)
    fill!(A, 0.0) # Pre-allocate
    # Left boundary
    A[1, 1] = 1.0
    # Interior grid points
    for i = 2:par.Nξo-1
        A[i, i] = 1.0 + (dτ/2)*( 2*par.D/(S^2*dξo^2) )
        A[i, i+1] = -(dτ/2)*( ξo[i]*u[end]/(2*S*dξo) + par.D/(S^2*dξo^2) )
        A[i, i-1] = -(dτ/2)*( -ξo[i]*u[end]/(2*S*dξo) + par.D/(S^2*dξo^2) )
    end
    # Right boundary
    A[end, end] = 1.0 + (dτ/2)*( 2*par.D/(S^2*dξo^2) )
    A[end, end-1] = -(dτ/2)*( 2*par.D/(S^2*dξo^2) )
    ### Build RHS ###
    b = Vector{Float64}(undef, par.Nξo) # Pre-allocate
    # Left boundary
    b[1] = gr
    # Interior grid points
    for i = 2:par.Nξo-1
        b[i] = gs[i] - (dτ/2)*( 2*par.D/(S^2*dξo^2) )*gs[i] + (dτ/2)*( ξo[i]*u[end]/(2*S*dξo) + par.D/(S^2*dξo^2) )*gs[i+1] + (dτ/2)*( -ξo[i]*u[end]/(2*S*dξo) + par.D/(S^2*dξo^2) )*gs[i-1]
    end
    # Right boundary
    b[end] = gs[end] - (dτ/2)*( 2*par.D/(S^2*dξo^2) )*gs[end] + (dτ/2)*( 2*par.D/(S^2*dξo^2) )*gs[end-1]
    ### Solve linear system ###
    return A\b
end