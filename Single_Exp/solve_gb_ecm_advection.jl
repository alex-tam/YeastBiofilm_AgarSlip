...
function solve_gb(par, dτ, dξ, ξ, h, ϕ, gs, gb, u, S)
    ...
    for i = 2:par.Nξ-1
        A[i, i] = par.Pe*h[i] + (dτ/2)*( par.Qb + par.Υ*ϕ[i]*h[i] + (h[i+1]+2*h[i]+h[i-1])/(2*S^2*dξ^2) + par.Pe*(1-ϕ[i])*u[i]*h[i]/(S*dξ) )
        A[i, i+1] = -(dτ/2)*( par.Pe*h[i]*ξ[i]*u[end]/(2*S*dξ) + (h[i+1]+h[i])/(2*S^2*dξ^2) )
        A[i, i-1] = -(dτ/2)*( -par.Pe*h[i]*ξ[i]*u[end]/(2*S*dξ) + (h[i]+h[i-1])/(2*S^2*dξ^2) + par.Pe*(1-ϕ[i-1])*u[i-1]*h[i-1]/(S*dξ) )
    end
    # Right boundary
    ...
    for i = 2:par.Nξ-1
        b[i] = par.Pe*h[i]*gb[i] + dτ*par.Qb*gs[i] - 
            (dτ/2)*( par.Qb + par.Υ*ϕ[i]*h[i] + (h[i+1]+2*h[i]+h[i-1])/(2*S^2*dξ^2) + par.Pe*(1-ϕ[i])*u[i]*h[i]/(S*dξ) )*gb[i] +
            (dτ/2)*( par.Pe*h[i]*ξ[i]*u[end]/(2*S*dξ) + (h[i+1]+h[i])/(2*S^2*dξ^2) )*gb[i+1] +
            (dτ/2)*( -par.Pe*h[i]*ξ[i]*u[end]/(2*S*dξ) + (h[i]+h[i-1])/(2*S^2*dξ^2) + par.Pe*(1-ϕ[i-1])*u[i-1]*h[i-1]/(S*dξ) )*gb[i-1]
    end
    # Right boundary
    b[end] = 0.0 # No-flux
    ### Solve linear system ###
