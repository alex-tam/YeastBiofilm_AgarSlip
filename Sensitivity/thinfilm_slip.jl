...
function thinfilm_slip(par, p)
    ...
    writedlm("h-par-$p-step-1.csv", h); 
    writedlm("phi-par-$p-step-1.csv", ϕ); 
    writedlm("gs-par-$p-step-1.csv", gs); 
    writedlm("xio-par-$p-step-1.csv", ξo); 
    writedlm("gso-par-$p-step-1.csv", gso); 
    writedlm("gb-par-$p-step-1.csv", gb); 
    writedlm("u-par-$p-step-1.csv", u)
    ### Time stepping ###
    for i = 1:par.Nτ-1
        ...
        if mod(i, par.plot_interval) == 0
            j = i+1
            writedlm("h-par-$p-step-$j.csv", h)
            writedlm("phi-par-$p-step-$j.csv", ϕ)
            writedlm("gs-par-$p-step-$j.csv", gs)
            writedlm("xio-par-$p-step-$j.csv", ξo)
            writedlm("gso-par-$p-step-$j.csv", gso)
            writedlm("gb-par-$p-step-$j.csv", gb)
            writedlm("u-par-$p-step-$j.csv", u)
            push!(plot_times, j)
            writedlm("plot_times.csv", plot_times)
        end
    end
    writedlm("S-par-$p.csv", S)
end